...
def process_file(args, transcriptor, filename):
    fn = os.path.basename(filename).replace('.mp3', '')
...
def main():
    ...
    # if args.dir_mode:
    #     filenames = [os.path.join(args.audio_path, f, 'vocals.mp3') for f in os.listdir(args.audio_path)]
    # else:
    ...
    if args.dir_mode:
        for f in tqdm(os.listdir(args.audio_path)):
            ...
            #     continue
            fn = f#os.path.basename(f).replace('.mp3','')
            output_midi_path = os.path.join(args.output_midi_path, fn+ ".mid")
            ...
            if os.path.exists(output_midi_path):
                continue
            # print('start')
            inference(args, transcriptor, os.path.join(args.audio_path, f,'vocals.mp3'), output_midi_path)
    else:
        audio_path = args.audio_path
        output_midi_path = args.output_midi_path
        inference(args, transcriptor, os.path.join(args.audio_path, f,'vocals.mp3'), output_midi_path)
...
if __name__ == '__main__':
    ...
    main()
    # mp3dir="accomponimentsandvocal"
    # if os.path.exists(mp3dir):
    #     files=os.listdir(mp3dir)
    #     for file in files:
    #         os.remove(os.path.join(mp3dir,file))
